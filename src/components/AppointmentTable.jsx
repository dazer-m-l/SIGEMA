...
const AppointmentTable = ({ appointments = [], onDelete, onEdit, searchTerm }) => {
            ...
            filteredAppointments.map((appointment, index) => (
              <tr key={appointment.id_cita || index}>
                ...
                <td className="px-3 py-4 whitespace-nowrap">
                  <button
                    onClick={() => onEdit(appointment)} // Llamar a la función de edición
                    className="text-blue-600 hover:text-blue-900 mr-4" // Aumentar margen derecho
                    title="Actualizar"
                  >
                    <Edit2 className="h-5 w-5" /> {/* Ícono de lápiz */}
                  </button>
                  <button
